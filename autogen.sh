...
if [ "$PATH_TO_AUTOGEN" = "\." ]; then
    PATH_TO_AUTOGEN=""
else
    PATH_TO_AUTOGEN="$PATH_TO_AUTOGEN/"
fi
NAME_OF_AUTOGEN="`basename $0`"
AUTOGEN_SH="${PATH_TO_AUTOGEN}$NAME_OF_AUTOGEN"
echo "PATH_TO_AUTOGEN: $PATH_TO_AUTOGEN"
echo "NAME_OF_AUTOGEN: $NAME_OF_AUTOGEN"
...
manual_autogen ( ) {
    ...
    # libtoolize #
    ##############
    echo "if libtoolize_needed"
    if libtoolize_needed ; then
        echo "libtoolize_needed TRUE"
...
# Pull in COPYING and INSTALL from /usr/share/
